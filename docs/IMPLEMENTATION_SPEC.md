...
数値を誤認すると無料枠の前提が崩れるため、経路ごとに分けて記す（2026 年時点・要再確認は §8）。
...
- **CPU 時間（経路で異なる・重要）**:
  ...
  - **Cron Triggers 経路**（collector の日次起動）: HTTP とは別枠。**wall-clock 15 分**、CPU も cron 経由は緩い。収集処理（fetch 待ちが大半）はこちらで動かす前提なので余裕がある。
...
**結論**: 収集（collector）は Cron 経路（15 分枠）で動かすため余裕。api/web は HTTP 経路（CPU 10 ms）なので処理を軽く保つ。Phase 1 で収集件数を絞るのは subrequest 50 と D1 queries 50/invocation の両制約を避けるため。
...
> **設計判断（重要）**: `AnthropicSitemapAdapter` は内部で **HTMLRewriter（Workers ランタイム API）** を使う。そのため `packages/core` は「Workers ランタイム前提のコードを含むライブラリ」になる（純 Node では動かない）。検証・実行は常に Worker（`wrangler dev`）経由で行う（§7 Step 1 参照）。
...
## 前提（Prerequisites）— Step 0 着手前に整える
...
- `GET /articles` を実装: `env.DB.prepare("SELECT source,title,url,published_at,summary FROM articles ORDER BY published_at DESC").all()` を JSON で返す。snake_case → camelCase の変換をここで行う
...
- **CPU 時間の経路差**: api/web は HTTP 経路で CPU 10 ms 上限。重い処理を載せない。collector は cron 経路（15 分枠）。§1 の数値が最新か、着手時に公式で再確認
...
- Cloudflare 無料枠（developers.cloudflare.com で確認）: Workers 10 万 req/日、**CPU は HTTP 経路 10 ms 固定（30 秒は Paid 既定）／Cron 経路は wall-clock 15 分**、subrequest 50/request、Cron Triggers 5 本、D1 5 GB（1 DB 500 MB・書込 10 万行/日・**50 queries/invocation**）、D1 で `ON CONFLICT` upsert 可、HTMLRewriter で `getAttribute('content')` によるメタ抽出可、Next.js は OpenNext 推奨
